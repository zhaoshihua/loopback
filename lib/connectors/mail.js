...
Mailer.send = function(options, fn) {
  ...
  if (debug.enabled || settings && settings.debug) {
    ...
    console.log(g.f('\t TO:%s', options.to));
    console.log(g.f('\t FROM:%s', options.from));
    console.log(g.f('\t SUBJECT:%s', options.subject));
    console.log(g.f('\t TEXT:%s', options.text));
    console.log(g.f('\t {{HTML:}}%s', options.html));
  }
